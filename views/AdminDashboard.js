...
function AdminDashboard({
    ...
    const NavButton = ({ id, icon, label, width = 'w-[60px]' }) => (
    ...
    const todosLosCamposExistentes = [...new Set([...camposFijos, ...todosLosAlumnos.map(a => a.campo), ...todasAsistencias.map(a => a.campo), ...historialAsistencias.map(h => h.campo)].filter(Boolean))].sort();
    ...
    if (vistaActual === 'inicio') {
        ...
        contenidoAdmin = (
            <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-300"><button onClick={() => setExpandirFiltroAdmin(!expandirFiltroAdmin)} className="w-full flex items-center justify-between p-6 bg-white hover:bg-slate-50 transition-colors"><div className="flex items-center"><div className="w-10 h-10 bg-sky-50 text-sky-500 rounded-xl flex items-center justify-center mr-3"><i className="fas fa-filter"></i></div><div className="text-left"><h3 className="font-bold text-slate-700 text-sm">Filtro Demográfico</h3><p className="text-[10px] text-slate-400">Analizar edades globales</p></div></div><i className={`fas fa-chevron-down text-slate-300 transition-transform duration-300 ${expandirFiltroAdmin ? 'rotate-180' : ''}`}></i></button>{expandirFiltroAdmin && (<div className="p-6 pt-0 animate-in slide-in-from-top-2 duration-200 border-t border-slate-50"><div className="flex space-x-3 mb-4 mt-4"><div className="w-1/2"><label className="text-[9px] font-bold text-slate-400 uppercase ml-1">Mínima (Años)</label><input type="number" className="w-full p-3 mt-1 bg-slate-50 rounded-xl outline-none border border-slate-100" value={edadMin} onChange={e=>setEdadMin(e.target.value)} /></div><div className="w-1/2"><label className="text-[9px] font-bold text-slate-400 uppercase ml-1">Máxima (Años)</label><input type="number" className="w-full p-3 mt-1 bg-slate-50 rounded-xl outline-none border border-slate-100" value={edadMax} onChange={e=>setEdadMax(e.target.value)} /></div></div>{(edadMin !== '' || edadMax !== '') && (() => { const filtrados = todosLosAlumnos.filter(a => { if (edadMin !== '' && a.edad < parseInt(edadMin)) return false; if (edadMax !== '' && a.edad > parseInt(edadMax)) return false; return true; }); const m = filtrados.filter(a => a.genero === 'M').length; const f = filtrados.filter(a => a.genero === 'F').length; return (<div className="flex justify-around items-center bg-sky-50 p-4 rounded-2xl mb-4"><div className="text-center"><p className="text-2xl font-black text-sky-600">{filtrados.length}</p><p className="text-[9px] font-bold text-sky-500 uppercase">Total</p></div><div className="text-center"><p className="text-2xl font-black text-indigo-500">{m}</p><p className="text-[9px] font-bold text-indigo-400 uppercase">Niños</p></div><div className="text-center"><p className="text-2xl font-black text-pink-500">{f}</p><p className="text-[9px] font-bold text-pink-400 uppercase">Niñas</p></div></div>); })()}</div>)}</div>
        ...
        contenidoAdmin = (
            <div className="space-y-4 animate-in slide-in-from-right duration-300">
                ...
                <div className="bg-sky-50 rounded-[32px] p-6 shadow-sm border border-sky-100">
                    ...
                    <div className="flex justify-around items-center bg-white p-4 rounded-2xl mt-4 shadow-sm"><div className="text-center"><p className="text-3xl font-black text-sky-600">{adminAlumnosFiltrados.length}</p><p className="text-[9px] font-bold text-sky-500 uppercase">Total Red</p></div><div className="w-px h-10 bg-slate-100"></div><div className="text-center"><p className="text-2xl font-black text-indigo-500">{adminAlumnosFiltrados.filter(a => a.genero === 'M').length}</p><p className="text-[9px] font-bold text-indigo-400 uppercase">Niños</p></div><div className="w-px h-10 bg-slate-100"></div><div className="text-center"><p className="text-2xl font-black text-pink-500">{adminAlumnosFiltrados.filter(a => a.genero === 'F').length}</p><p className="text-[9px] font-bold text-pink-400 uppercase">Niñas</p></div></div>
                </div>
                            ...
                            return (
                                <div key={campo} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                                    <div className="flex justify-between items-center mb-3">
                                        ...
                                        <div className="flex items-center space-x-2"><span className="bg-indigo-50 text-indigo-600 font-black text-[10px] px-2 py-1.5 rounded uppercase">{total} Alumnos</span><button onClick={() => setCampoResetUI(campoResetUI === campo ? null : campo)} className="w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-500 rounded-lg hover:bg-sky-500 hover:text-white transition-colors"><i className="fas fa-cog"></i></button><button onClick={() => onDeleteCampo(campo)} className="w-8 h-8 flex items-center justify-center bg-rose-50 text-rose-500 rounded-lg hover:bg-rose-500 hover:text-white transition-colors"><i className="fas fa-trash-alt"></i></button></div>
                                    </div>
                                    <div className="mt-2"><div className="flex justify-between text-[10px] font-bold text-slate-500 mb-1"><span>Material: Parte {prog.parte} • {prog.impartidas}/25 dadas</span><span className="text-indigo-500">{prog.porc}%</span></div><div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden"><div className="bg-indigo-500 h-2 rounded-full transition-all duration-1000" style={{width: `${prog.porc}%`}}></div></div></div>
                                    {campoResetUI === campo && (
                                        <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 animate-in slide-in-from-top-2 duration-200"><p className="text-[10px] font-black text-slate-500 mb-3 uppercase tracking-widest text-center"><i className="fas fa-sync-alt mr-1"></i> Reiniciar Material</p><div className="flex space-x-2"><button onClick={() => { onResetLecciones(campo, 0); setCampoResetUI(null); }} className="flex-1 py-3 bg-indigo-500 text-white rounded-lg text-xs font-bold shadow-md active:scale-95 transition-all">Empezar Parte 1</button><button onClick={() => { onResetLecciones(campo, 25); setCampoResetUI(null); }} className="flex-1 py-3 bg-sky-500 text-white rounded-lg text-xs font-bold shadow-md active:scale-95 transition-all">Empezar Parte 2</button></div></div>
                                    )}
                                </div>
                            );
                        }
                    ...
                    camposConHistorial.map(campo => {
                        ...
                        return (
                            <div key={campo} className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-300">
                                <button onClick={() => setCampoHistorialExp(isExpanded ? null : campo)} className="w-full flex items-center justify-between p-5 bg-white hover:bg-slate-50 transition-colors"><div className="flex items-center"><div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center mr-3"><i className="fas fa-map-marker-alt"></i></div><div className="text-left"><h3 className="font-bold text-slate-700 text-sm">{campo}</h3><p className="text-[10px] text-slate-400">{registrosCampo.length} clases impartidas</p></div></div><i className={`fas fa-chevron-down text-slate-300 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}></i></button>
                                {isExpanded && (<div className="p-4 pt-0 animate-in slide-in-from-top-2 duration-200 border-t border-slate-50 bg-slate-50/50"><div className="space-y-2 mt-4 max-h-[300px] overflow-y-auto pr-1">{registrosCampo.map((h, i) => (<div key={i} className="bg-white p-3 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm"><div><p className="font-black text-slate-700 text-sm">{formatoFecha(h.fecha)}</p><p className="text-[9px] text-slate-400 uppercase mt-0.5"><i className="fas fa-user mr-1"></i>{h.maestro}</p>{h.leccion !== undefined && (<p className={`text-[10px] font-bold mt-1 ${h.leccionImpartida ? 'text-indigo-500' : 'text-rose-500'}`}><i className="fas fa-book-open mr-1"></i>Lección {h.leccion} {h.leccionImpartida ? '✅' : '❌'}</p>)}</div><div className="flex space-x-1 text-[10px] font-bold"><span className="bg-emerald-50 text-emerald-600 px-2 py-1 rounded-lg">P: {h.totales?.presentes || 0}</span><span className="bg-rose-50 text-rose-600 px-2 py-1 rounded-lg">A: {h.totales?.ausentes || 0}</span></div></div>))}</div></div>)}
                            </div>
                        );
                    })}
                </div>
                                ...
                                {personalLogistica.length === 0 ? <p className="text-center text-slate-400 text-sm italic mt-8">No hay personal de logística registrado.</p> :
                                    personalLogistica.map(p => (
                                        <div key={p.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                                            <div className="flex items-center space-x-3 w-1/2"><div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-bold text-sm">{p.nombre.charAt(0)}</div><p className="font-bold text-slate-700 text-sm truncate">{p.nombre}</p></div>
                                            <div className="w-1/2">
                                                <select defaultValue={p.grupo || ''} onChange={(e) => onAssignGroup(p.id, e.target.value)} className={`w-full p-2 rounded-xl text-xs font-bold outline-none border ${p.grupo ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'bg-rose-50 border-rose-100 text-rose-500'}`}>
                                                    <option value="">-- Sin Grupo --</option><option value="Grupo 1">Grupo 1</option><option value="Grupo 2">Grupo 2</option><option value="Grupo 3">Grupo 3</option><option value="Grupo 4">Grupo 4</option><option value="Grupo 5">Grupo 5</option>
                                                </select>
                                            </div>
                                        </div>
                                    ))
    ...
    return (
        <>
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-slate-100 flex justify-around items-center p-2 z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                <NavButton id="inicio" icon="fa-home" label="Panel" width="w-[60px]" />
                <NavButton id="poblacion" icon="fa-layer-group" label="Campos" width="w-[60px]" />
                <NavButton id="historial" icon="fa-history" label="Historial" width="w-[60px]" />
                <NavButton id="logistica" icon="fa-truck" label="Logística" width="w-[60px]" />
                <NavButton id="personal" icon="fa-address-book" label="Personal" width="w-[60px]" />
            </div>
        </>
    );
}
