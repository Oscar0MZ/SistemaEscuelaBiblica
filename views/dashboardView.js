            ...
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm">
                    ...
                    <div className="flex justify-around text-center divide-x divide-slate-50">
                        <div className="px-2"><p className="text-3xl font-black text-emerald-500">{totalPresentes}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Presentes</p></div>
                        <div className="px-2"><p className="text-3xl font-black text-rose-500">{totalAusentes}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Ausentes</p></div>
                        <div className="px-2"><p className="text-3xl font-black text-amber-500">{totalPermisos}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Permisos</p></div>
                    </div>
                </div>
                        ...
                        <div className="p-6 pt-0 animate-in slide-in-from-top-2 duration-200 border-t border-slate-50">
                            <div className="space-y-3 mt-4">
                                {camposOrdenados.length > 0 ? camposOrdenados.map(campo => {
                                    ...
                                    const hayAsistencia = reportesCampo.length > 0;
                                        ...
                                        <div key={campo} className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                            ...
                                            {hayAsistencia ? (
                                                <div className="flex space-x-2">
                                                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold">P: {p}</span>
                                                    <span className="text-[10px] bg-rose-100 text-rose-700 px-2 py-0.5 rounded font-bold">A: {a}</span>
                                                    <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-bold">Perm: {perm}</span>
                                                </div>
                                            ) : <p className="text-[9px] text-slate-400 italic"><i className="fas fa-clock mr-1"></i> Sin asistencia registrada</p>}
                                        </div>
                                    );
