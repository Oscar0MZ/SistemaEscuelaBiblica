...
function DashboardView({ 
    ...
    usuario, 
    datosUsuarioActual, // Necesitamos esto para el nombre
    ...
    if (usuario === 'MAESTRO' || usuario === 'AUXILIAR') {
        ...
        // Formateo del nombre para bienvenida
            ...
            return (
                <div className="flex flex-col h-full space-y-4 pt-4 animate-in fade-in duration-500">
                    <div className="px-2 mb-2">
                        ...
                        <h2 className="text-2xl font-black text-slate-800">Hola, {rolDisplay} {nombreDisplay}</h2>
                        <p className="text-slate-400 text-xs">Resumen del día</p>
                    </div>
                    
                    {/* 2. TARJETA ASISTENCIA (LÓGICA MEJORADA) */}
                    <button 
                        onClick={() => !estaBloqueada && setVistaActual('asistencia')} 
                        className={`w-full p-6 rounded-[32px] text-left relative overflow-hidden group transition-all shadow-lg 
                            ${estaBloqueada ? 'bg-slate-100 border border-slate-200' : 
                              asistenciaTomada ? 'bg-white border border-slate-100 active:scale-95' : 'bg-rose-500 text-white shadow-rose-200 active:scale-95'}`}
                    >
                        {asistenciaTomada ? (
                            // SI YA SE TOMÓ (SEA MÍA O DE OTRO)
                            <>
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className={`font-bold text-sm flex items-center ${estaBloqueada ? 'text-slate-500' : 'text-slate-700'}`}>
                                        <i className={`fas ${estaBloqueada ? 'fa-lock' : 'fa-clipboard-check'} mr-2 ${estaBloqueada ? 'text-slate-400' : 'text-emerald-500'}`}></i> 
                                        {estaBloqueada ? 'Asistencia (Solo Lectura)' : 'Asistencia Hoy'}
                                    </h3>
                                    {estaBloqueada ? (
                                        <span className="text-[9px] bg-slate-200 text-slate-500 px-2 py-1 rounded-lg font-bold uppercase">Bloqueado</span>
                                    ) : (
                                        <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg font-bold">TUYA</span>
                                    )}
                                </div>
                                
                                {/* MOSTRAR SIEMPRE LOS NÚMEROS (INCLUSO SI ESTÁ BLOQUEADO) */}
                                <div className="flex justify-around text-center">
                                    <div>
                                        <p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-emerald-500'}`}>{asistenciaHoy.totales.presentes}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase">Presentes</p>
                                    </div>
                                    <div>
                                        <p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-rose-500'}`}>{asistenciaHoy.totales.ausentes}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase">Ausentes</p>
                                    </div>
                                    <div>
                                        <p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-amber-500'}`}>{asistenciaHoy.totales.permisos}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase">Permisos</p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-4 text-center text-[10px] font-bold uppercase tracking-widest">
                                    {estaBloqueada ? (
                                        <span className="text-slate-400">Tomada por: {asistenciaHoy.maestro}</span>
                                    ) : (
                                        <span className="text-indigo-500">Toca para editar</span>
                                    )}
                                </div>
                            </>
                        ) : (
                    ...
                    <button onClick={() => setVistaActual('gestion')} className="w-full bg-indigo-600 p-6 rounded-[32px] text-white shadow-xl shadow-indigo-200 text-left relative overflow-hidden group active:scale-95 transition-all">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-10 rounded-bl-[100px] pointer-events-none group-hover:scale-110 transition-transform"></div>
                        <div className="flex justify-between items-start"><div><p className="text-xs font-bold uppercase opacity-70 tracking-widest">Mis Alumnos</p><p className="text-5xl font-black tracking-tighter mt-1">{alumnos.length}</p></div><div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-2xl backdrop-blur-sm"><i className="fas fa-users"></i></div></div>
