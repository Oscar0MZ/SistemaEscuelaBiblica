...
function DashboardView({ 
    ...
    todosLosAlumnos = [], 
    asistenciaHoy, 
    todasAsistenciasHoy = [], // <--- Recibimos las asistencias de todos los campos
    usuario, 
    ...
    if (esAdmin) {
        ...
        // 1. Calcular Población por Campo
        ...
        // 2. Calcular Asistencia Global (Suma de todos los reportes)
        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                {/* --- NUEVA TARJETA: RESUMEN ASISTENCIA GLOBAL --- */}
                <div className="bg-white rounded-[32px] border border-slate-100 p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-[10px] bg-slate-100 text-slate-400 font-bold px-2 py-1 rounded-lg uppercase">{new Date().toLocaleDateString()}</span>
                    </div>
                    <div className="flex justify-around text-center divide-x divide-slate-50">
                        <div className="px-2">
                            <p className="text-2xl font-black text-emerald-500">{totalPresentes}</p>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Presentes</p>
                        </div>
                        <div className="px-2">
                            <p className="text-2xl font-black text-rose-500">{totalAusentes}</p>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Ausentes</p>
                        </div>
                        <div className="px-2">
                            <p className="text-2xl font-black text-amber-500">{totalPermisos}</p>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Permisos</p>
                        </div>
                    </div>
                </div>

                {/* POBLACIÓN ESTUDIANTIL (ACORDEÓN MEJORADO) */}
                ...
                {/* DIRECTORIO PERSONAL (Igual que antes) */}
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-300">
        ...
        if (vistaActual === 'asistencia') {
            return (
                <div className="flex flex-col h-full pt-4 animate-in slide-in-from-right duration-300">
                    <div className="flex items-center space-x-4 mb-4 px-2"><button onClick={() => setVistaActual('inicio')} className="w-10 h-10 bg-white border border-slate-100 rounded-xl text-slate-400 hover:text-indigo-600 flex items-center justify-center"><i className="fas fa-arrow-left"></i></button><div><h2 className="text-xl font-black text-slate-800">Pasar Lista</h2><p className="text-slate-400 text-xs">{new Date().toLocaleDateString()}</p></div></div>
