...
function DashboardView({ 
    ...
    const formatoFecha = (f) => f ? `${f.split('-')[2]}/${f.split('-')[1]}` : '';
    const textoFechas = rango ? `${formatoFecha(rango.inicio)} - ${formatoFecha(rango.fin)}` : 'Calculando...';
    ...
    // COMPONENTE DE BOTÓN PARA LA BARRA INFERIOR
    const NavButton = ({ id, icon, label }) => (
        <button 
            onClick={() => setVistaActual(id)} 
            className={`flex flex-col items-center justify-center w-16 h-14 rounded-2xl transition-all ${vistaActual === id ? 'text-indigo-600 bg-indigo-50 font-black' : 'text-slate-400 hover:text-slate-600 font-bold'}`}
        >
            <i className={`fas ${icon} text-xl mb-1 ${vistaActual === id ? 'animate-bounce' : ''}`}></i>
                ...
                <div className="space-y-4 animate-in slide-in-from-right duration-300">
                    ...
                    <div className="bg-sky-50 rounded-[32px] p-6 shadow-sm border border-sky-100">
                        ...
                        <div className="flex space-x-4">
                            <div className="w-1/2">
                                <label className="text-[10px] font-bold text-sky-600 uppercase ml-2">Mínima (Años)</label>
                                <input type="number" placeholder="Ej: 0" className="w-full p-4 mt-1 bg-white rounded-2xl outline-none border border-sky-100 focus:ring-2 focus:ring-sky-300 text-lg font-bold" value={edadMin} onChange={e=>setEdadMin(e.target.value)} />
                            </div>
                            <div className="w-1/2">
                                <label className="text-[10px] font-bold text-sky-600 uppercase ml-2">Máxima (Años)</label>
                                <input type="number" placeholder="Ej: 5" className="w-full p-4 mt-1 bg-white rounded-2xl outline-none border border-sky-100 focus:ring-2 focus:ring-sky-300 text-lg font-bold" value={edadMax} onChange={e=>setEdadMax(e.target.value)} />
                            </div>
                        </div>

                        {/* RESULTADO GLOBAL DEL FILTRO */}
                        <div className="flex justify-around items-center bg-white p-4 rounded-2xl mt-4 shadow-sm">
                            ...
                            <div className="text-center"><p className="text-2xl font-black text-indigo-500">{adminTotalNinos}</p><p className="text-[9px] font-bold text-indigo-400 uppercase">Niños (M)</p></div>
                            <div className="w-px h-10 bg-slate-100"></div>
                            <div className="text-center"><p className="text-2xl font-black text-pink-500">{adminTotalNinas}</p><p className="text-[9px] font-bold text-pink-400 uppercase">Niñas (F)</p></div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pb-24">
                        {camposDisponibles.map(campo => {
                            // Si hay filtro, calculamos M/F para este rango. Si no, mostramos Asistencia y Total.
                            if (filtroActivo) {
                                ...
                                return (
                                    <div key={campo} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                                        ...
                                        <div className="flex space-x-2 text-xs font-bold">
                                            <span className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg">T: {total}</span>
                                            <span className="bg-sky-50 text-sky-600 px-3 py-1.5 rounded-lg">M: {ninos}</span>
                                            <span className="bg-pink-50 text-pink-600 px-3 py-1.5 rounded-lg">F: {ninas}</span>
                                        </div>
                                    </div>
                                );
                            } else {
                                // Vista normal: Total Niños + Asistencia
                                ...
                                return (
                                    <div key={campo} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                                        ...
                                        ) : <p className="text-[10px] text-slate-400 italic"><i className="fas fa-clock mr-1"></i> Sin asistencia registrada</p>}
                                    </div>
                                );
                            }
                        })}
                ...
                <div className="space-y-4 animate-in slide-in-from-right duration-300 h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pb-24 pr-2 mt-4">
                        {listaAdminVisible.map(m => (
                            <div key={m.id} className="flex items-center justify-between p-4 bg-white rounded-2xl shadow-sm border border-slate-100 transition-colors">
                                <div className="flex items-center space-x-4"><div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center font-bold text-lg">{m.nombre.charAt(0)}</div><div><p className="font-bold text-slate-700 text-sm">{m.nombre}</p><span className="text-[10px] text-slate-400 font-bold uppercase">{m.clase} - {m.campo || 'N/A'}</span></div></div>
                                <div className="flex space-x-2"><button onClick={() => onEdit(m)} className="text-indigo-400 w-10 h-10 flex items-center justify-center bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-colors"><i className="fas fa-edit"></i></button><button onClick={() => onDelete(m)} className="text-rose-400 w-10 h-10 flex items-center justify-center bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors"><i className="fas fa-trash"></i></button></div>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    if (usuario === 'MAESTRO' || usuario === 'AUXILIAR') {
                ...
                <div className="flex flex-col h-full pt-4 animate-in slide-in-from-right duration-300">
                    ...
                    <div className="bg-sky-50 rounded-[32px] p-6 shadow-sm border border-sky-100 mb-6">
                        <div className="flex space-x-4 mb-5">
                            <div className="w-1/2">
                                <label className="text-[10px] font-bold text-sky-600 uppercase ml-2">Edad Mínima</label>
                                <input type="number" placeholder="Ej: 0" className="w-full p-4 mt-1 bg-white rounded-2xl outline-none border border-sky-100 focus:ring-2 focus:ring-sky-300 text-xl font-black text-slate-700 text-center" value={edadMin} onChange={e=>setEdadMin(e.target.value)} />
                            </div>
                            <div className="w-1/2">
                                <label className="text-[10px] font-bold text-sky-600 uppercase ml-2">Edad Máxima</label>
                                <input type="number" placeholder="Ej: 5" className="w-full p-4 mt-1 bg-white rounded-2xl outline-none border border-sky-100 focus:ring-2 focus:ring-sky-300 text-xl font-black text-slate-700 text-center" value={edadMax} onChange={e=>setEdadMax(e.target.value)} />
                            </div>
                        </div>

                        {(edadMin !== '' || edadMax !== '') && (
                            <div className="flex justify-around items-center bg-white p-4 rounded-2xl shadow-sm animate-in zoom-in duration-200">
                                <div className="text-center"><p className="text-3xl font-black text-indigo-500">{alumnosFiltrados.length}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Total</p></div>
                                <div className="w-px h-10 bg-slate-100"></div>
                                <div className="text-center"><p className="text-2xl font-black text-sky-500">{totalNinos}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Niños (M)</p></div>
                                <div className="w-px h-10 bg-slate-100"></div>
                                <div className="text-center"><p className="text-2xl font-black text-pink-500">{totalNinas}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Niñas (F)</p></div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex-1 bg-white rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] border-t border-slate-100 p-6 overflow-hidden flex flex-col">
                        <h3 className="text-sm font-bold text-slate-700 mb-4 px-2">Resultados del Filtro</h3>
                        <div className="overflow-y-auto space-y-3 pb-24 pr-2">
                            {alumnosFiltrados.length === 0 ? (
                                ...
                                <div key={nino.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    <div className="flex items-center space-x-3">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold border border-slate-100 shadow-sm ${nino.genero === 'M' ? 'bg-sky-50 text-sky-600' : nino.genero === 'F' ? 'bg-pink-50 text-pink-600' : 'bg-white text-slate-400'}`}>{nino.nombre.charAt(0)}</div>
                                        <div>
                                            <p className="font-bold text-slate-700 text-sm">{nino.nombre}</p>
                                            <p className="text-[10px] text-slate-400 font-bold tracking-wide mt-0.5"><i className="fas fa-birthday-cake mr-1 text-rose-300"></i>{nino.edad} Años</p>
                                        </div>
                                    </div>
                                </div>
                            )))}
        ...
        return (
            <>
                {contenidoMaestro}
                {/* BARRA DE NAVEGACIÓN INFERIOR (MAESTRO) */}
                <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-slate-100 flex justify-around items-center p-2 z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                    ...
                    <NavButton id="filtros" icon="fa-filter" label="Filtros" />
                </div>
            </>
        );
    }
