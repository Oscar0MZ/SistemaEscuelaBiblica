        ...
        if (vistaActual === 'inicio') {
            return (
                <div className="flex flex-col h-full space-y-4 pt-4 animate-in fade-in duration-500">
                    <div className="px-2 mb-2"><h2 className="text-2xl font-black text-slate-800">Hola, {usuario.toLowerCase()}</h2><p className="text-slate-400 text-xs">Resumen del día</p></div>
                    <button onClick={() => !estaBloqueada && setVistaActual('asistencia')} className={`w-full p-6 rounded-[32px] text-left relative overflow-hidden group transition-all shadow-lg ${estaBloqueada ? 'bg-slate-100 border border-slate-200 cursor-not-allowed opacity-80' : asistenciaTomada ? 'bg-white border border-slate-100 active:scale-95' : 'bg-rose-500 text-white shadow-rose-200 active:scale-95'}`}>
                        {estaBloqueada ? (
                            <><div className="flex justify-between items-center mb-2"><h3 className="font-bold text-slate-500 flex items-center"><i className="fas fa-lock mr-2"></i> Asistencia Bloqueada</h3></div><p className="text-xs text-slate-400">Tomada por: <b>{asistenciaHoy.maestro}</b></p><p className="text-[10px] text-rose-400 mt-1">Solo el autor puede editarla.</p></>
                        ) : asistenciaTomada ? (
                            <><div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-700 flex items-center"><i className="fas fa-clipboard-check text-emerald-500 mr-2"></i> Asistencia Hoy</h3><span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg font-bold">TUYA</span></div><div className="flex justify-around text-center"><div><p className="text-2xl font-black text-emerald-500">{asistenciaHoy.totales.presentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Presentes</p></div><div><p className="text-2xl font-black text-rose-500">{asistenciaHoy.totales.ausentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Ausentes</p></div><div><p className="text-2xl font-black text-amber-500">{asistenciaHoy.totales.permisos}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Permisos</p></div></div><div className="mt-4 text-center text-[10px] text-indigo-500 font-bold uppercase tracking-widest">Toca para editar</div></>
