        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-300">
                    ...
                    {expandirPoblacion && (
                        <div className="p-6 pt-0 animate-in slide-in-from-top-2 duration-200 border-t border-slate-50">
                            <div className="grid grid-cols-2 gap-3 mt-4">{camposOrdenados.length > 0 ? camposOrdenados.map(campo => (<div key={campo} className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex justify-between items-center"><span className="text-xs font-bold text-slate-600 truncate mr-2 max-w-[80px]">{campo}</span><span className="text-sm font-black text-indigo-500">{conteoPorCampo[campo]}</span></div>)) : <p className="col-span-2 text-center text-xs text-slate-400 italic py-2">No hay alumnos registrados.</p>}</div>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-300">
                    ...
                    {expandirPersonal && (
                        <div className="p-6 pt-0 animate-in slide-in-from-top-2 duration-200 border-t border-slate-50">
                            <div className="flex items-center bg-slate-50 rounded-2xl px-4 py-3 my-4"><i className="fas fa-search text-slate-300 mr-3"></i><input type="text" placeholder="Buscar maestro..." className="bg-transparent w-full outline-none text-sm" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} /></div>
                                ...
                                {listaAdminVisible.map(m => (
                                    <div key={m.id} className="flex items-center justify-between p-2 rounded-xl hover:bg-slate-50 transition-colors">
                                        ...
                                        <div className="flex space-x-1">
                                            ...
                                            {/* AQUÍ PASAMOS 'm' COMPLETO AL BORRAR */}
                                            <button onClick={() => onDelete(m)} className="text-rose-400 w-8 h-8 flex items-center justify-center hover:bg-rose-50 rounded-lg transition-colors"><i className="fas fa-trash"></i></button>
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-indigo-600 p-6 rounded-[32px] text-white shadow-xl shadow-indigo-200 flex flex-col justify-between h-40 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-20 h-20 bg-white opacity-10 rounded-bl-[100px] pointer-events-none"></div><p className="text-xs font-bold uppercase opacity-70 tracking-widest">Personal Activo</p><div><p className="text-5xl font-black tracking-tighter">{activos.length}</p><p className="text-[10px] opacity-70 mt-1">Miembros Totales</p></div>
                    </div>
