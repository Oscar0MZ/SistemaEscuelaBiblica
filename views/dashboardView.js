...
function DashboardView({ 
    ...
    usuario, 
    datosUsuarioActual, // <--- Recibimos al usuario logueado
    ...
    // --- VISTA MAESTRO / AUXILIAR (CON LÓGICA DE BLOQUEO) ---
    if (usuario === 'MAESTRO' || usuario === 'AUXILIAR') {
        // Verificar si la asistencia está bloqueada para este usuario
        ...
        if (vistaActual === 'inicio') {
            return (
                <div className="flex flex-col h-full space-y-4 pt-4 animate-in fade-in duration-500">
                    ...
                    {/* TARJETA ASISTENCIA (CON BLOQUEO) */}
                    <button 
                        ...
                        className={`w-full p-6 rounded-[32px] text-left relative overflow-hidden group transition-all shadow-lg 
                            ${estaBloqueada ? 'bg-slate-100 border border-slate-200 cursor-not-allowed opacity-80' : 
                        ...
                        {estaBloqueada ? (
                            ...
                            <>
                                <div className="flex justify-between items-center mb-2">
                                    <h3 className="font-bold text-slate-500 flex items-center"><i className="fas fa-lock mr-2"></i> Asistencia Bloqueada</h3>
                                </div>
                                <p className="text-xs text-slate-400">Tomada por: <b>{asistenciaHoy.maestro}</b></p>
                                <p className="text-[10px] text-rose-400 mt-1">Solo el autor puede editarla.</p>
                            </>
                        ) : asistenciaTomada ? (
                            // CASO COMPLETADA (AUTOR)
                            <>
                                <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-700 flex items-center"><i className="fas fa-clipboard-check text-emerald-500 mr-2"></i> Asistencia Hoy</h3><span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg font-bold">TUYA</span></div>
                                <div className="flex justify-around text-center"><div><p className="text-2xl font-black text-emerald-500">{asistenciaHoy.totales.presentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Presentes</p></div><div><p className="text-2xl font-black text-rose-500">{asistenciaHoy.totales.ausentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Ausentes</p></div><div><p className="text-2xl font-black text-amber-500">{asistenciaHoy.totales.permisos}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Permisos</p></div></div>
