    ...
    if (usuario === 'MAESTRO' || usuario === 'AUXILIAR') {
        ...
        if (vistaActual === 'inicio') {
            return (
                <div className="flex flex-col h-full space-y-4 pt-4 animate-in fade-in duration-500">
                    <div className="px-2 mb-2"><h2 className="text-2xl font-black text-slate-800">Hola, {rolDisplay} {nombreDisplay}</h2><p className="text-slate-400 text-xs">Resumen del día</p></div>
                    <button onClick={() => !estaBloqueada && setVistaActual('asistencia')} className={`w-full p-6 rounded-[32px] text-left relative overflow-hidden group transition-all shadow-lg ${estaBloqueada ? 'bg-slate-100 border border-slate-200' : asistenciaTomada ? 'bg-white border border-slate-100 active:scale-95' : 'bg-rose-500 text-white shadow-rose-200 active:scale-95'}`}>
                        {asistenciaTomada ? (
                            <><div className="flex justify-between items-center mb-4"><h3 className={`font-bold text-sm flex items-center ${estaBloqueada ? 'text-slate-500' : 'text-slate-700'}`}><i className={`fas ${estaBloqueada ? 'fa-lock' : 'fa-clipboard-check'} mr-2 ${estaBloqueada ? 'text-slate-400' : 'text-emerald-500'}`}></i> {estaBloqueada ? 'Asistencia (Solo Lectura)' : 'Asistencia Hoy'}</h3>{estaBloqueada ? (<span className="text-[9px] bg-slate-200 text-slate-500 px-2 py-1 rounded-lg font-bold uppercase">Bloqueada</span>) : (<span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg font-bold">TUYA</span>)}</div><div className="flex justify-around text-center"><div><p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-emerald-500'}`}>{asistenciaHoy.totales.presentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Presentes</p></div><div><p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-rose-500'}`}>{asistenciaHoy.totales.ausentes}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Ausentes</p></div><div><p className={`text-2xl font-black ${estaBloqueada ? 'text-slate-500' : 'text-amber-500'}`}>{asistenciaHoy.totales.permisos}</p><p className="text-[10px] font-bold text-slate-400 uppercase">Permisos</p></div></div><div className="mt-4 text-center text-[10px] font-bold uppercase tracking-widest">{estaBloqueada ? (<span className="text-slate-400">Tomada por: {asistenciaHoy.maestro}</span>) : (<span className="text-indigo-500">Toca para editar</span>)}</div></>
                    ...
                    <button onClick={() => setVistaActual('gestion')} className="w-full bg-indigo-600 p-6 rounded-[32px] text-white shadow-xl shadow-indigo-200 text-left relative overflow-hidden group active:scale-95 transition-all"><div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-10 rounded-bl-[100px] pointer-events-none group-hover:scale-110 transition-transform"></div><div className="flex justify-between items-start"><div><p className="text-xs font-bold uppercase opacity-70 tracking-widest">Mis Alumnos</p><p className="text-5xl font-black tracking-tighter mt-1">{alumnos.length}</p></div><div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-2xl backdrop-blur-sm"><i className="fas fa-users"></i></div></div><div className="mt-4 flex items-center text-xs font-bold text-indigo-200"><span>Gestionar lista y Filtros</span><i className="fas fa-arrow-right ml-2 group-hover:translate-x-1 transition-transform"></i></div></button>
                </div>
            );
        }
        ...
        return (
            <div className="flex flex-col h-full pt-4 animate-in slide-in-from-right duration-300">
                ...
                <button onClick={onOpenAlumnoModal} className="w-full bg-emerald-500 p-4 rounded-[24px] shadow-lg shadow-emerald-200 active:scale-95 transition-all text-white flex items-center justify-between mb-4"><div className="flex items-center space-x-3"><div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-lg backdrop-blur-sm"><i className="fas fa-plus"></i></div><span className="font-bold">Nuevo Alumno</span></div><i className="fas fa-chevron-right opacity-50"></i></button>
                        ...
                        {alumnosFiltrados.length === 0 ? (
                            ...
                            <div key={nino.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                <div className="flex items-center space-x-4">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border border-slate-100 shadow-sm ${nino.genero === 'M' ? 'bg-sky-50 text-sky-600' : nino.genero === 'F' ? 'bg-pink-50 text-pink-600' : 'bg-white text-slate-400'}`}>
                                        {nino.nombre.charAt(0)}
                                    </div>
                                    <div>
                                        <p className="font-bold text-slate-700 text-sm">{nino.nombre}</p>
                                        <p className="text-[10px] text-slate-400 font-bold tracking-wide"><i className="fas fa-birthday-cake mr-1 text-rose-300"></i>{nino.edad} Años <span className="mx-1 text-slate-300">|</span> <span className={nino.genero === 'M' ? 'text-sky-500' : 'text-pink-500'}>{nino.genero === 'M' ? 'Niño' : nino.genero === 'F' ? 'Niña' : '-'}</span></p>
                                    </div>
                                </div>
