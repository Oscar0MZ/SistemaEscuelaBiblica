...
function App() {
    ...
    const handleBorrarAlumno = async () => { if (!idAlumnoBorrar) return; try { await AlumnosService.eliminar(idAlumnoBorrar); setIdAlumnoBorrar(null); alert("Alumno eliminado."); } catch (e) { alert("Error"); } };
    ...
    // --- ACTUALIZADO: GUARDAR ASISTENCIA CON LECCIÓN ---
    const handleGuardarAsistencia = async (registros, leccion, leccionImpartida) => { 
        ...
        try { 
            await AlumnosService.guardarAsistencia({ 
                ...
                leccion: leccion, // Se guarda la lección
                leccionImpartida: leccionImpartida, // Se guarda si se dio o no
                timestamp: Date.now() 
            }); 
            alert("Asistencia y Lección guardadas"); 
            return true; 
