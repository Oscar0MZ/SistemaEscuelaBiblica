...
function App() {
    ...
    const handleGuardarAlumno = async (e) => {
        ...
        if (!nombre || !fecha || !genero) { alert("Por favor completa todos los campos (Nombre, Fecha y Género)"); return; }
    ...
    // --- ELIMINAR ALUMNO (MENSAJE DE CONFIRMACIÓN DE BORRADO GLOBAL) ---
    const handleBorrarAlumno = async () => { 
        if (!idAlumnoBorrar) return; 
        try { 
            await AlumnosService.eliminar(idAlumnoBorrar); 
            setIdAlumnoBorrar(null); 
            // Esto confirma al maestro que se eliminó de todos lados
            alert("Alumno eliminado del sistema global."); 
