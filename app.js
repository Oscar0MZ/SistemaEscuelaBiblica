...
function App() {
    ...
    // NUEVO: HISTORIAL DE ASISTENCIAS (INMUTABLE)
    ...
    // CARGA DE DATOS (AHORA INCLUYE HISTORIAL)
    useEffect(() => {
        ...
        if (usuario === 'ADMIN') {
            ...
            const unsub3 = AlumnosService.suscribirHistorialGlobal(setHistorialAsistencias); // NUEVO
            ...
            const unsub3 = AlumnosService.suscribirHistorialPorCampo(datosUsuarioActual.campo, setHistorialAsistencias); // NUEVO
            return () => { unsub1(); unsub2(); unsub3(); };
        }
    }, [usuario, datosUsuarioActual]);
    ...
    const handleBorrarAlumno = async () => { if (!idAlumnoBorrar) return; try { await AlumnosService.eliminar(idAlumnoBorrar); setIdAlumnoBorrar(null); alert("Alumno eliminado del sistema global."); } catch (e) { alert("Error al eliminar alumno"); } };
        ...
        <div className="flex flex-col h-screen max-w-md mx-auto bg-white shadow-2xl overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-5 pb-24 bg-slate-50/50 scroll-smooth">
                <DashboardView 
                    ...
                    historialAsistencias={historialAsistencias} // <--- PASAMOS EL HISTORIAL A LA VISTA
